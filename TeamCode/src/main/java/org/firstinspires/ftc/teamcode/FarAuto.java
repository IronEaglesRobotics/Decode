    ...
    public void runAuto() {
        switch (pathState) {
            ...
            case 2: //is the robot done launching? if yes, go to pickup 1 and stop shooter
                if (getRuntime() > timer) {
                    robot.intake.close();
                    follower().followPath(getPickup1, .75, true);
...
//                    follower().followPath(getPickup2, true);
    ...
    public void loop() {
        ...
        power = robot.getShooter().calculateShooterPower(distance + 3);
        hood = robot.getShooter().calculateHoodPose(distance + 3);
        robot.getShooter().setShot(power, hood);
        robot.getTurret().aim(-1 + robot.getTurret().calculateAngle(config.getGoalPose().getX(), config.getGoalPose().getY(), robot.getFollower().getPose().getX(), robot.getFollower().getPose().getY()), Math.toDegrees(robot.getFollower().getHeading()));
        runAuto();
